...
int translation(char* str_prog, char* ptr);

int htod(char* val);

char vartonum(char* str);
...
int main(int argc, char* argv[])
{
    ...
    int end_code = translation(str_prog, cpu_code);
    for(int j = 0; j < labels.counter; j++)
    {
        printf("%s %d\n", labels.labels[j].name, labels.labels[j].addr);
    }

    translation(str_prog, cpu_code);
    ...
    else
    {
        ...
        strcpy(asm_name, argv[1]);

        strcpy((asm_name+len), ASM_FILE_EXTENSION);
...
int translation(char* prog, char* ptr)
{
    ISNOTNULL(prog);
    ISNOTNULL(ptr);
        ...
        if (!strcmp(cmd, S_CMD[i - BEGIN]))
        {
            ...
            *ptr = i;
            ...
            if (i == MOV)
            {
                ...
                if (!strcmp(str, S_REG[0])) {*ptr = REG; ptr++; *ptr = AX; ptr++;}
                else
                if (!strcmp(str, S_REG[1])) {*ptr = REG; ptr++; *ptr = BX; ptr++;}
                else
                if (!strcmp(str, S_REG[2])) {*ptr = REG; ptr++; *ptr = CX; ptr++;}
                else
                if (!strcmp(str, S_REG[3])) {*ptr = REG; ptr++; *ptr = DX; ptr++;}
                else
                {
                    *ptr = VAR;
                    ptr++;
...
char vartonum(char* str)
{
    ...
    while (i < vars.counter)
    {
        if (!strcmp(str, vars.vars[i]))
        {
            return i;}
        else
            i++;
    ...
    if (i < MAX_VAR)
    {
        char* var = (char*) calloc(1, ARG_MAX);
        strcpy(var, str);
        vars.vars[i] = var;
        //printf("var %s is %d\n", var, vars.counter);
        //getchar();
